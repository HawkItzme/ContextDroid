...
- **Automatiquement suggéré** quand Codex détecte ces patterns d'erreur :
  - `rtk: command not found` → RTK non installé ou pas dans PATH
  - Hook errors in Codex → Hooks mal configurés ou non exécutables
...
if [ -f ".Codex/hooks/rtk-rewrite.sh" ]; then
    echo "✅ OK: rtk-rewrite.sh hook present"
    # Check if hook is executable
    if [ -x ".Codex/hooks/rtk-rewrite.sh" ]; then
...
if [ -f ".Codex/hooks/rtk-suggest.sh" ]; then
    echo "✅ OK: rtk-suggest.sh hook present"
    if [ -x ".Codex/hooks/rtk-suggest.sh" ]; then
        echo "✅ OK: hook is executable"
    else
        echo "⚠️ WARNING: hook not executable (chmod +x needed)"
    fi
...
if [ -n "$CLAUDE_CODE_HOOK_BASH_TEMPLATE" ]; then
    echo "✅ OK: Running in Codex context"
    echo "   Hook env var set: CLAUDE_CODE_HOOK_BASH_TEMPLATE"
else
    echo "⚠️ WARNING: Not running in Codex (hooks won't activate)"
...
🎯 Codex Context:  ✅ OK (hook environment detected)
                    ⚠️ WARNING (not in Codex)
...
options:
  ...
  - label: "chmod +x .Codex/hooks/bash/*.sh"
...
**IMPORTANT** : Codex doit suggérer `/diagnose` automatiquement quand il voit :
...
### Issue : Hooks not triggering in Codex

**Symptom**: Commands not rewritten to `rtk <cmd>` automatically

**Diagnosis**:
```bash
# Check if in Codex context
echo $CLAUDE_CODE_HOOK_BASH_TEMPLATE
# Should print hook template path - if empty, not in Codex
...
# Verify hooks load in new Codex session
# (restart Codex session after chmod)
```
